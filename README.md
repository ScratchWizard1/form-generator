...
- [Receiving & Validating Data](#-receiving--validating-data)
  ...
  - [Save Rule Behavior](#save-rule-behavior)
...
- File upload validation (size, MIME type, extension) with optional file saving
...
| Save | `sa`, `sav`, `save` | optional | Saves the uploaded file to the specified directory. Path is relative to project root. Can be provided as value or via `FG_UPLOAD_PATH` in `.env`. **Must be placed last in the rule chain.** | `save:uploads/files;` |
...
---
### Save Rule Behavior
- If no path is provided in the rule → upload path is loaded from the `.env` file.
- If no `.env` path is configured → validation fails with an error.
- If a path is provided → it is used directly.
- Path is always resolved relative to the project root.
- Target directory is created automatically if it does not exist.
- If a file with the same name already exists, the new file is saved with a numeric suffix
  (e.g. `photo.jpg` → `photo (2).jpg` → `photo (3).jpg`), preserving all existing files.
- The returned file array reflects the **actual saved filename**, which may differ from the original.
- **Must be placed last in the rule chain** — after all other file validations (e.g. `mime`, `size`, `extension`).

Example `.env` key:

```env
FG_UPLOAD_PATH=uploads/files
```
---
...
---
## 🛡 File Validation
FormGenerator validates uploaded files and optionally saves them using the `save` rule.
...
Optionally saves:
- Use the `save` rule to move the uploaded file to a target directory
- If `save` is not used, you are responsible for storing files yourself
...
---
### Score Threshold
reCAPTCHA Enterprise returns a risk score between `0.0` (likely bot) and `1.0` (likely human). The minimum accepted score can be configured via `.env`:

```env
FG_RECAPTCHA_SCORE = 0.5
```

- Default value is `0.5`
- Accepted range is `0.0` – `1.0` (values outside this range are automatically clamped)
...
**`FG_UPLOAD_PATH`**
Default upload directory for the `save` validation rule. The path is resolved relative to the project root. Can be overridden per-field by providing a value directly in the rule (e.g. `save:uploads/images`). See [Save Rule Behavior](#save-rule-behavior) for details.
...
**`FG_RECAPTCHA_SCORE`**
Minimum accepted reCAPTCHA risk score. Must be a value between `0.0` and `1.0`. Values outside this range are automatically clamped. Defaults to `0.5`. See [Score Threshold](#score-threshold) for details.
