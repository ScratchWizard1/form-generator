...
use FormGenerator\Validation\Rules\Save;
...
class Validation
{
    ...
    protected static function isValidWeek(
     ...
     /**
 * Validate if the given data is valid for saving.
 *
 * @param string|array{name: string, type: string, tmp_name: string, error: int, size: int} $data
 * @param string $key
 * @param array{test: string, modification?: string, value?: array<string>} $test
 * @param bool $devMessage Whether to include developer-level error details
 * @throws \FormGenerator\ErrorHandling\FormGeneratorException
 * @return ValidationResult
 */
    protected static function isValidSave(
        string|array $data,
        string $key,
        array $test,
        bool $devMessage = false
    ): ValidationResult {
        $result = Save::check($data, $test, $devMessage);
        if (is_array($result)) {
            return new ValidationResult(['value' => $result]);
        } elseif ($result === true) {
            return new ValidationResult(['value' => $data]);
    ...
    protected static function assocArrayToString(array $data, string $parentKey = ''): string
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (is_array($value)) {
                if (is_numeric(array_key_first($value))) {
                    $parts[] = $currentKey . ':' . implode(
                        '\x1F',
                        array_map(fn(mixed $v): string => Globals::toStr($v), $value)
                    );
                } else {
        ...
        return implode(
            "\x7C",
            array_map(fn(mixed $v): string => Globals::toStr($v), $parts)
        );
    }
