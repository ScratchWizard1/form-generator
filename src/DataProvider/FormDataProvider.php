...
class FormDataProvider
{
    ...
    private static function testCaptcha(
        ...
        try {
            ...
            if ($credentials) {
                $credentials = self::joinPaths($projectRoot, $credentials);
            }

            if (!$credentials && $userProvidedCredentials) {
                $credentials = self::joinPaths($projectRoot, $userProvidedCredentials);
            }

            if (!$credentials) {
                $defaultPath = self::joinPaths($projectRoot, 'config/google-recaptcha.json');
            ...
            $rawScore  = Config::getWithFallback(
                ['FG_RECAPTCHA_SCORE', 'RECAPTCHA_SCORE'],
                0.5
            );
            $rawScore = is_string($rawScore) ? (string) $rawScore : "0.5";
            $rawScore = str_replace(',', '.', $rawScore);
            $acceptScore = is_numeric($rawScore) ? (float) $rawScore : 0.5;
            $acceptScore = max(0.0, min(1.0, $acceptScore));
